...
원본 프로젝트(2020.10.19 ~ 2020.11.16, Spring 5 + MyBatis + JSP)를 최신 스택으로 현대화하는 프로젝트다. 목표와 단계별 로드맵은 [`MODERNIZATION_PLAN.md`](./MODERNIZATION_PLAN.md)에 정리되어 있다.
...
이번 리팩토링은 핵심 대표 모듈(인증, 도서 카탈로그, 장바구니/구매/재고, 리뷰) 4개만 깊게 개선한다. 아래 모듈은 `legacy/DEV24Test`에 그대로 남아있고 신규 스택으로 포팅하지 않는다.
